...
<header id="header" class="header d-flex align-items-center fixed-top">
	...
	<table class="listbox" id="listbox">
			...
			<c:forEach var="list" items="${movielist}">
				<tr>	
				<c:if test="${loginId=='admin'}">		
					<td style="color: black;" >${list.MCode}</td>
					</c:if>
					<td><a href="mView?mCode=${list.MCode}">${list.MName}</a></td>
				</tr>
			</c:forEach>
		
			<tr>
				<th colspan="2" id="paging1">
				<!-- 페이징 처리 -->
				
				<!-- [이전] -->
				<c:if test="${paging.page <= 1}">[이전]</c:if>
				<c:if test="${paging.page > 1}"><a href="movielist?page=${paging.page-1}">[이전]</a></c:if>
				<!-- [페이지 번호] -->
				<c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage}">
					<c:if test="${paging.page == i}">${i}</c:if>
					<c:if test="${paging.page != i}"><a href="movielist?page=${i}">${i}</a></c:if>
				</c:forEach>
				<!-- [다음] -->
				<c:if test="${paging.page >= paging.maxPage}">[다음]</c:if>
				<c:if test="${paging.page < paging.maxPage}"><a href="movielist?page=${paging.page +1}">[다음]</a></c:if>
				</th>
			</tr>
			<tr>
			<td colspan="5">
			...
			<form action="movieSearch" method="GET">
				...
				<!-- 카테고리 -->
				<select name="category" class="listbox1">
					<option value="mName" >영화 이름</option>
					<option value="mAge" >관람연령</option>
					<option value="mCategory" >장르</option>
				...
				<!-- 키워드 -->
				<input type="text" name="keyword" class="listbox2"/>
