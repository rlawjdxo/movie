...
<header id="header" class="header d-flex align-items-center fixed-top">
	...
	<table class="listbox" id="listbox">
		...
		<c:forEach var="list" items="${relist}">
			<tr>
				<td><a href="reView?reserveId=${list.reserveId}">${list.reserveId}</a></td>
				<td>${list.emsCode}</td>
				<td>${list.MCode}</td>
				<td>${list.MName}</td>
				<td>${list.emgCode}</td>
			...
			<td colspan="8">
				...
				<form action="reSearch" method="GET">
					...
					<select name="category" class="listbox2">
						<option value="reserveId">예약번호</option>
						<option value="emgCode">상영정보코드</option>
